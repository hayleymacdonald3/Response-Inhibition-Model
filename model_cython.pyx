...
def get_fac(np.ndarray[np.float32_t, ndim=1] t, params):
    '''
    ...
    cdef np.ndarray[np.float32_t, ndim=1] res = np.zeros_like(t)
    cdef float temp
    cdef int i
...
def get_trials(np.ndarray[np.float32_t, ndim=1] t, params, int n_rep=10000):
    '''
    ...
    cdef int i, j
    cdef np.ndarray[np.float32_t, ndim=2]res = np.zeros((n_rep, t.size))
